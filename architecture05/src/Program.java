...
public class Program {
    ...
    public static void main(String[] args) {
        ...
        while (f) {
            ...
            System.out.println("6. Выполните рендер всех моделей");
            System.out.println("7. Выполнить рендер модели");
            System.out.println("8. Добавить модель");
            System.out.println("0. ЗАВЕРШИТЕ РАБОТЫ ПРИЛОЖЕНИЯ");
...
class Editor3D implements UILayer {
    ...
    private void checkProjectFile() {
        if (projectFile == null) {
            throw new RuntimeException("Файл проекта не определено.");
        }
    }
    ...
    public void renderModel(int i) {
        ...
        if (i < 0 || i > models.size() - 1) {
            throw new RuntimeException("Номер модели указан некорретно");
        }
        System.out.println("Подождите...");
        long startTime = System.currentTimeMillis();
    ...
    public void addModel() {
        businessLogicalLayer.addModels();
    }
}
...
class EditorDatabaseAccess implements DatabaseAccess {
    ...
    @Override
    public void addModel() {
        editorDatabase.addModel(new Model3D());
    }
